...
data_folder = "data"
results_file = "llm_results_codellama_34b.json"
...
def query_ollama(prompt, model="codellama:34b"):
...
def main():
    ...
    for filename in sorted(os.listdir(data_folder)):
        ...
        print(f"[codellama:34b] {filename} → raw: {answer} → label: {label}")

        # Save after each file (so we don’t lose progress)
        with open(results_file, "w") as f:
            json.dump(results, f, indent=4)

    print(f" Finished codellama:34b, saved results to {results_file}")
